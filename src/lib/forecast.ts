...
export interface GoalProjection {
  status: GoalStatus;
  /** Trailing-30-day total of the metric today. */
  currentValue: number;
  /** Projected trailing-30-day total on the target date (null = not enough data). */
...
/**
 * A goal means: "the trailing 30-day total of this metric should reach
 * `targetValue` by `targetDate`". Projects that trailing total by combining
 * observed values with the forecast.
 */
export function projectGoal(
  ...
  const series = toContiguousSeries(rows);
  const observed = series.filter((p) => p.date <= today);
  const currentValue = observed
    .slice(-30)
  ...
  const forecast = forecastSeries(observed, daysRemaining);
  if (!forecast) {
    return {
      status: "at_risk",
      currentValue,
  ...
  // Trailing-30d window ending on the target date, over observed + forecast.
  const combined = [...observed, ...forecast.points].filter(
    (p) => p.date <= targetDate,
  );
  const projectedValue = combined
    .slice(-30)
    .reduce((total, p) => total + p.value, 0);
  ...
  const status: GoalStatus =
    projectedValue >= targetValue
      ? "on_track"
      : projectedValue >= targetValue * 0.85
        ? "at_risk"
        : "off_track";

  return { status, currentValue, projectedValue, targetValue, daysRemaining };
}
